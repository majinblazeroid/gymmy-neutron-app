  ...
  if (exercises.length === 0) {
    return (
      <div className="pt-8 pb-6 space-y-6">
        ...
        <button onClick={onFinish} className="w-full rounded-2xl py-5 font-semibold text-[#495057]" style={{ background: "#adf7b6" }}>
          Finish Workout
        </button>
      </div>
    );
    ...
    <div className="pt-8 pb-6 space-y-6">
      ...
      <div className="h-1 bg-gray-100 rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${progress}%`, background: "#adf7b6" }}
        />
      </div>
      ...
      <div className="flex gap-3">
        ...
        {isLast ? (
          ...
          <button
            onClick={() => setCurrentIndex((i) => i + 1)}
            className="flex-1 rounded-2xl py-4 font-semibold text-sm transition-opacity shadow-sm active:opacity-75 text-[#495057] flex items-center justify-center gap-2"
            style={{ background: "#adf7b6" }}
          >
            Next
          ...
          return (
            <button
              ...
              className={`rounded-full transition-all ${
                i === currentIndex ? "w-5 h-2"
                : hasSets          ? "w-2 h-2 bg-gray-400"
                                   : "w-2 h-2 bg-gray-200"
              }`}
              style={i === currentIndex ? { background: "#adf7b6" } : undefined}
            />
          );
        })}
      </div>
